...
async function start(projectPath, port) {
  ...
  try {
    ...
    if (!jsonConfig.rootPath) {
      exit(
        `请先初始化项目，终端执行：${chalk.yellow("lds init project-name")}`
      );
    }
    ...
    webpackConfig.plugins.push(
      new HtmlWebpackPlugin({
        ...
        template:
          pageConfig.frame === "normal"
            ? path.resolve(absolutePath, "./index.html")
            : htmlTplPath,
    ...
    const devServerOptions = {
      contentBase: [
        contentPath,
        path.resolve(jsonConfig.rootPath, "developer"),
      ],
    ...
    server.listen(port, "localhost", function (err, result) {
      ...
      console.log(
        logSymbols.success,
        chalk.green(`项目启动成功，浏览器打开: http://localhost:${port}`)
      );
    });
  } catch (error) {
